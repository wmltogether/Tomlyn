...
namespace Tomlyn.SourceGeneration
{
    ...
    public sealed class TomlSerializerContextGenerator : IIncrementalGenerator
    {
        private static readonly DiagnosticDescriptor ContextMustBePartial = new(
            "TOMLYN001",
            "Toml serializer context must be partial",
            "Type '{0}' derives from Tomlyn.Serialization.TomlSerializerContext and must be declared partial to support source generation",
        ...
        private static readonly DiagnosticDescriptor UnsupportedMemberType = new(
            ...
            "Type '{0}' contains member '{1}' of unsupported type '{2}'. Add [TomlSerializable(typeof({2}))] to the context or change the member type.",
            "Tomlyn.SourceGeneration",
            DiagnosticSeverity.Error,
            true);
        ...
        private static readonly DiagnosticDescriptor UnsupportedDictionaryKeyType = new(
            ...
            "Type '{0}' contains member '{1}' of dictionary-like type '{2}' with non-string keys. TOML table keys must be strings.",
            "Tomlyn.SourceGeneration",
            DiagnosticSeverity.Error,
            true);
        ...
        private static readonly DiagnosticDescriptor InvalidSourceGenerationOption = new(
            "TOMLYN005",
            "Invalid source generation option",
            "Invalid source generation option on context '{0}': {1}",
            "Tomlyn.SourceGeneration",
            DiagnosticSeverity.Error,
            true);
        ...
        private static readonly DiagnosticDescriptor InvalidExtensionDataMember = new(
            "TOMLYN006",
            "Invalid extension data member",
            "Type '{0}' extension data member '{1}' is invalid: {2}",
            "Tomlyn.SourceGeneration",
            DiagnosticSeverity.Error,
            true);
        ...
        private static readonly DiagnosticDescriptor InvalidPolymorphismConfiguration = new(
            "TOMLYN007",
            "Invalid polymorphism configuration",
            "Type '{0}' polymorphism configuration is invalid: {1}",
            "Tomlyn.SourceGeneration",
            DiagnosticSeverity.Error,
            true);
        ...
        private static readonly DiagnosticDescriptor JsonSerializableNotSupported = new(
            "TOMLYN008",
            "JsonSerializable is not supported on TOML contexts",
            "Type '{0}' derives from Tomlyn.Serialization.TomlSerializerContext and uses [JsonSerializable]. Replace [JsonSerializable(...)] with [TomlSerializable(...)] on the context.",
            "Tomlyn.SourceGeneration",
        ...
        private const string TomlSourceGenerationOptionsAttributeMetadataName =
            "Tomlyn.Serialization.TomlSourceGenerationOptionsAttribute";
        ...
        public void Initialize(IncrementalGeneratorInitializationContext context)
        {
            var candidateContexts = context.SyntaxProvider
                .CreateSyntaxProvider(static (node, _) => node is ClassDeclarationSyntax,
                    static (ctx, _) => TryCreateContextModel(ctx))
        ...
        private static void EmitContext(SourceProductionContext context, Compilation compilation, ContextModel model)
        {
            ...
            {
                context.ReportDiagnostic(Diagnostic.Create(JsonSerializableNotSupported,
                    model.ContextSymbol.Locations.FirstOrDefault(), model.ContextSymbol.ToDisplayString()));
            }
            ...
            if (!model.IsValid)
            {
                context.ReportDiagnostic(Diagnostic.Create(ContextMustBePartial,
                    model.ContextSymbol.Locations.FirstOrDefault(), model.ContextSymbol.ToDisplayString()));
                return;
            }
            ...
            builder.Append("    private ").Append(model.TypeName)
                .AppendLine("(TomlSerializerOptions options, bool _generated) : base(options) { }");
            builder.AppendLine();

            builder.Append("    public static ").Append(model.TypeName)
                .AppendLine(" Default { get; } = new(CreateDefaultOptions(), _generated: true);");
            builder.AppendLine();
        ...
        private static void EmitCreateDefaultOptions(StringBuilder builder, ContextModel model)
        {
            ...
            if (!model.Options.ConverterTypes.IsDefaultOrEmpty)
            {
                ...
                {
                    builder.Append("            new ")
                        .Append(converterType.ToDisplayString(SymbolDisplayFormat.FullyQualifiedFormat)).AppendLine("(),");
                }
            ...
            {
                builder.Append("        options = options with { PropertyNamingPolicy = ")
                    .Append(model.Options.PropertyNamingPolicyExpression).AppendLine(" };");
            }

            if (!string.IsNullOrEmpty(model.Options.DictionaryKeyPolicyExpression))
            {
                builder.Append("        options = options with { DictionaryKeyPolicy = ")
                    .Append(model.Options.DictionaryKeyPolicyExpression).AppendLine(" };");
            }

            if (model.Options.PropertyNameCaseInsensitive is not null)
            {
                builder.Append("        options = options with { PropertyNameCaseInsensitive = ")
                    .Append(model.Options.PropertyNameCaseInsensitive.Value ? "true" : "false").AppendLine(" };");
            }

            if (model.Options.DefaultIgnoreCondition is not null &&
                TryGetTomlIgnoreConditionExpression(model.Options.DefaultIgnoreCondition.Value,
                    out var ignoreConditionExpression))
            {
                builder.Append("        options = options with { DefaultIgnoreCondition = ")
                    .Append(ignoreConditionExpression).AppendLine(" };");
            }

            if (model.Options.DuplicateKeyHandling is not null &&
                TryGetTomlDuplicateKeyHandlingExpression(model.Options.DuplicateKeyHandling.Value,
                    out var duplicateKeyHandlingExpression))
            {
                builder.Append("        options = options with { DuplicateKeyHandling = ")
                    .Append(duplicateKeyHandlingExpression).AppendLine(" };");
            }

            if (model.Options.MaxDepth is not null)
            {
                builder.Append("        options = options with { MaxDepth = ")
                    .Append(model.Options.MaxDepth.Value.ToString(CultureInfo.InvariantCulture)).AppendLine(" };");
            }

            if (model.Options.MappingOrder is not null &&
                TryGetTomlMappingOrderPolicyExpression(model.Options.MappingOrder.Value, out var mappingOrderExpression))
            {
                builder.Append("        options = options with { MappingOrder = ").Append(mappingOrderExpression)
                    .AppendLine(" };");
            }

            if (model.Options.DottedKeyHandling is not null &&
                TryGetTomlDottedKeyHandlingExpression(model.Options.DottedKeyHandling.Value,
                    out var dottedKeyHandlingExpression))
            {
                builder.Append("        options = options with { DottedKeyHandling = ").Append(dottedKeyHandlingExpression)
                    .AppendLine(" };");
            }

            if (model.Options.WriteIndented is not null)
            {
                builder.Append("        options = options with { WriteIndented = ")
                    .Append(model.Options.WriteIndented.Value ? "true" : "false").AppendLine(" };");
            }

            if (model.Options.IndentSize is not null)
            {
                builder.Append("        options = options with { IndentSize = ")
                    .Append(model.Options.IndentSize.Value.ToString(CultureInfo.InvariantCulture)).AppendLine(" };");
            }

            if (model.Options.NewLine is not null &&
                TryGetTomlNewLineKindExpression(model.Options.NewLine.Value, out var newLineExpression))
            {
                builder.Append("        options = options with { NewLine = ").Append(newLineExpression).AppendLine(" };");
            }

            if (model.Options.RootValueHandling is not null &&
                TryGetTomlRootValueHandlingExpression(model.Options.RootValueHandling.Value,
                    out var rootValueHandlingExpression))
            {
                builder.Append("        options = options with { RootValueHandling = ").Append(rootValueHandlingExpression)
                    .AppendLine(" };");
            }

            if (!string.IsNullOrWhiteSpace(model.Options.RootValueKeyName))
            {
                builder.Append("        options = options with { RootValueKeyName = \"")
                    .Append(EscapeStringLiteral(model.Options.RootValueKeyName!)).AppendLine("\" };");
            }

            if (model.Options.InlineTablePolicy is not null &&
                TryGetTomlInlineTablePolicyExpression(model.Options.InlineTablePolicy.Value, out var inlineTableExpression))
            {
                builder.Append("        options = options with { InlineTablePolicy = ").Append(inlineTableExpression)
                    .AppendLine(" };");
            }

            if (model.Options.TableArrayStyle is not null &&
                TryGetTomlTableArrayStyleExpression(model.Options.TableArrayStyle.Value, out var tableArrayExpression))
            {
                builder.Append("        options = options with { TableArrayStyle = ").Append(tableArrayExpression)
                    .AppendLine(" };");
            }
        ...
        private static void EmitTypeInfoProperty(StringBuilder builder, SourceProductionContext context, ContextModel model,
            ITypeSymbol type)
        {
            ...
            var publicPropertyName = TryGetCustomTypeInfoPropertyName(model, type, out var customPropertyName)
                ? customPropertyName
                : propertyName;
            var propertyAccessibility = string.Equals(publicPropertyName, propertyName, StringComparison.Ordinal)
                ? "public"
                : "private";
            var typeName = type.ToDisplayString(SymbolDisplayFormat.FullyQualifiedFormat);

            builder.Append("    private TomlTypeInfo<").Append(typeName).Append(">? _").Append(propertyName)
                .AppendLine(";");
            builder.Append("    ").Append(propertyAccessibility).Append(" TomlTypeInfo<").Append(typeName).Append("> ")
                .Append(propertyName).AppendLine();
            builder.Append("        => _").Append(propertyName).Append(" ??= Create").Append(propertyName)
                .AppendLine("();");
            builder.AppendLine();

            if (!string.Equals(publicPropertyName, propertyName, StringComparison.Ordinal))
            {
                builder.Append("    public TomlTypeInfo<").Append(typeName).Append("> ").Append(publicPropertyName)
                    .Append(" => ").Append(propertyName).AppendLine(";");
                builder.AppendLine();
            }

            builder.Append("    private TomlTypeInfo<").Append(typeName).Append("> Create").Append(propertyName)
                .AppendLine("()");
            builder.AppendLine("    {");
            ...
            else if (TryGetSequenceElementType(type, out var enumerableElementType, out var kind))
            {
                ...
                {
                    builder.Append(
                        "        throw new global::System.InvalidOperationException(\"Unsupported sequence kind.\");");
                }
            }
            ...
            else if (TryGetPolymorphicShape(context, model, type, out var polymorphic, false))
            {
                builder.Append("        global::Tomlyn.TomlTypeInfo<").Append(typeName)
                    .AppendLine(">? __baseTypeInfo = null;");
                if (TryGetPocoShape(context, model, type, out _))
                {
                    builder.Append("        __baseTypeInfo = new __TomlTypeInfo_").Append(propertyName)
                        .AppendLine("(this);");
                }

                builder.AppendLine(
                    "        var __derivedTypeInfoByDiscriminator = new global::System.Collections.Generic.Dictionary<string, global::Tomlyn.TomlTypeInfo>(global::System.StringComparer.Ordinal)");
                builder.AppendLine("        {");
                foreach (var derived in polymorphic.DerivedTypes)
                {
                    if (derived.Discriminator is null)
                    {
                        continue; // skip default derived type in dictionary
                    }

                    builder.Append("            [\"").Append(EscapeStringLiteral(derived.Discriminator)).Append("\"] = ")
                        .Append(GetTypeInfoPropertyName(derived.Type)).AppendLine(",");
                }
        ...
        private static void EmitPocoTypeInfo(StringBuilder builder, ContextModel model, ITypeSymbol type, PocoShape poco)
        {
            ...
            builder.AppendLine();
            builder.Append("    private sealed class __TomlTypeInfo_").Append(propertyName).Append(" : TomlTypeInfo<")
                .Append(typeName).AppendLine(">");
            builder.AppendLine("    {");
            builder.Append("        private readonly ").Append(model.TypeName).AppendLine(" _context;");
            builder.AppendLine();
            builder.Append("        public __TomlTypeInfo_").Append(propertyName).Append("(").Append(model.TypeName)
                .AppendLine(" context) : base(context.Options)");
            builder.AppendLine("        {");
            ...
            builder.AppendLine("        {");
            builder.AppendLine(
                "            if (value is null) throw new TomlException(\"TOML does not support null values.\");");
            if (callsOnSerializing)
            {
                builder.AppendLine(
                    "            ((global::Tomlyn.Serialization.ITomlOnSerializing)value).OnTomlSerializing();");
            }
            ...
            if (poco.ExtensionData is { } extensionData)
            {
                ...
                builder.AppendLine("            {");
                builder.AppendLine(
                    "                __usedKeys = new global::System.Collections.Generic.HashSet<string>(Options.PropertyNameCaseInsensitive ? global::System.StringComparer.OrdinalIgnoreCase : global::System.StringComparer.Ordinal);");
                builder.AppendLine("            }");
            }
            ...
            for (var i = 0; i < poco.Members.Length; i++)
            {
                ...
                builder.Append("            var __member").Append(i.ToString(CultureInfo.InvariantCulture))
                    .Append(" = value.").Append(member.MemberName).AppendLine(";");
            }
            ...
            if (poco.ExtensionData is { } extensionDataWrite)
            {
                ...
                builder.AppendLine("                    }");
                builder.AppendLine(
                    "                    if (__usedKeys is not null && __usedKeys.Contains(__key)) throw new TomlException($\"Extension data key '{__key}' conflicts with an existing member key.\");");
                builder.AppendLine("                    writer.WritePropertyName(__key);");
                builder.Append("                    _context.").Append(extensionValueTypeInfo).Append(".Write(writer, ")
                    .Append(writeValueArgument).AppendLine(");");
                builder.AppendLine("                }");
                builder.AppendLine("            }");
            }
            ...
            {
                builder.AppendLine(
                    "            ((global::Tomlyn.Serialization.ITomlOnSerialized)value).OnTomlSerialized();");
            }
            ...
            if (constructor is { ErrorMessage: not null } ctorError)
            {
                ...
                builder.AppendLine("        {");
                builder.AppendLine(
                    "            if (reader.TokenType != TomlTokenType.StartTable) throw reader.CreateException($\"Expected {TomlTokenType.StartTable} token but was {reader.TokenType}.\");");
                builder.Append("            throw new TomlException(\"")
                    .Append(EscapeStringLiteral(ctorError.ErrorMessage!)).AppendLine("\");");
                builder.AppendLine("        }");
            }
            else if (poco.RequiresGeneratedObjectInitializer)
            {
                var bufferedConstructor = constructor ?? new PocoConstructor(null,
                    ImmutableArray<PocoConstructorParameter>.Empty, null, false);
                EmitPocoReadWithConstructor(builder, model, type, poco, bufferedConstructor, readReturnType,
                    callsOnDeserializing, callsOnDeserialized, true);
            }
            else if (constructor is { IsValid: true } ctor && !ctor.Parameters.IsDefaultOrEmpty)
            {
                EmitPocoReadWithConstructor(builder, model, type, poco, ctor, readReturnType, callsOnDeserializing,
                    callsOnDeserialized, false);
            }
            else
            {
                builder.Append("        public override ").Append(readReturnType).AppendLine(" Read(TomlReader reader)");
                ...
                {
                    builder.AppendLine(
                        "            ((global::Tomlyn.Serialization.ITomlOnDeserializing)value).OnTomlDeserializing();");
                }
                ...
                if (poco.Members.Length <= 64)
                {
                    ...
                    if (hasRequiredMembers)
                    {
                        ...
                        builder.Append("            const ulong requiredMask = ")
                            .Append(requiredMask.ToString(CultureInfo.InvariantCulture)).AppendLine("UL;");
                    }
                }
                else
                {
                    ...
                    builder.Append(") ? new bool[").Append(poco.Members.Length.ToString(CultureInfo.InvariantCulture))
                        .AppendLine("] : null;");
                }
                ...
                builder.AppendLine("            {");
                builder.AppendLine(
                    "                if (reader.TokenType != TomlTokenType.PropertyName) throw reader.CreateException($\"Expected {TomlTokenType.PropertyName} token but was {reader.TokenType}.\");");
                builder.AppendLine("                if (Options.PropertyNameCaseInsensitive)");
                builder.AppendLine("                {");
                builder.AppendLine("                    var name = reader.PropertyName!;");
                builder.AppendLine("                    reader.Read();");
                EmitMemberDispatch(builder, poco, true);
                builder.AppendLine("                }");
                builder.AppendLine("                else");
                builder.AppendLine("                {");
                EmitMemberDispatch(builder, poco, false);
                builder.AppendLine("                }");
                builder.AppendLine("            }");
                ...
                if (hasRequiredMembers)
                {
                    ...
                    {
                        builder.AppendLine(
                            "            if (requiredMask != 0 && (seenMask & requiredMask) != requiredMask)");
                        builder.AppendLine("            {");
                        ...
                        for (var i = 0; i < poco.Members.Length; i++)
                        {
                            ...
                            builder.Append("                if ((seenMask & (1UL << ")
                                .Append(i.ToString(CultureInfo.InvariantCulture)).AppendLine(")) == 0)");
                            builder.AppendLine("                {");
                            builder.Append(
                                    "                    throw span is { } locatedSpan ? new TomlException(locatedSpan, $\"Missing required TOML key '")
                    ...
                    else
                    {
                        ...
                        for (var i = 0; i < poco.Members.Length; i++)
                        {
                            ...
                            var serializedName = EscapeStringLiteral(poco.Members[i].SerializedName);
                            builder.Append("                if (!seen[").Append(i.ToString(CultureInfo.InvariantCulture))
                                .AppendLine("])");
                            builder.AppendLine("                {");
                            builder.Append(
                                    "                    throw span is { } locatedSpan ? new TomlException(locatedSpan, $\"Missing required TOML key '")
                ...
                {
                    builder.AppendLine(
                        "            ((global::Tomlyn.Serialization.ITomlOnDeserialized)value).OnTomlDeserialized();");
                }
            ...
            bool useObjectInitializerConstruction)
        {
            ...
            for (var i = 0; i < ctor.Parameters.Length; i++)
            {
                ...
                builder.Append("            ").Append(parameterTypeName).Append(" __arg")
                    .Append(i.ToString(CultureInfo.InvariantCulture)).Append(" = ").Append(defaultLiteral).AppendLine(";");
                builder.Append("            bool __argSeen").Append(i.ToString(CultureInfo.InvariantCulture))
                    .AppendLine(" = false;");
            }
            ...
            for (var i = 0; i < poco.Members.Length; i++)
            {
                ...
                builder.Append("            ").Append(memberTypeName).Append(" __memberValue")
                    .Append(i.ToString(CultureInfo.InvariantCulture)).Append(" = ").Append(defaultLiteral).AppendLine(";");
                builder.Append("            bool __memberSeen").Append(i.ToString(CultureInfo.InvariantCulture))
                    .AppendLine(" = false;");
            }

            var extensionData = poco.ExtensionData;
            if (extensionData is not null)
            {
                ...
                builder.Append("            global::System.Collections.Generic.Dictionary<string, ")
                    .Append(extensionValueTypeName).AppendLine(">? __extensionData = null;");
            }
            ...
            for (var i = 0; i < ctor.Parameters.Length; i++)
            {
                ...
                builder.Append("                    ").Append(prefix).Append("(string.Equals(name, \"")
                    .Append(EscapeStringLiteral(parameter.KeyName)).Append("\", ").Append(comparison).AppendLine("))");
                builder.AppendLine("                    {");
                builder.Append("                        if (__argSeen").Append(i.ToString(CultureInfo.InvariantCulture))
                    .AppendLine(
                        " && Options.DuplicateKeyHandling == TomlDuplicateKeyHandling.Error) throw reader.CreateException($\"Duplicate key '{name}' was encountered.\");");
                builder.Append("                        __argSeen").Append(i.ToString(CultureInfo.InvariantCulture))
                    .AppendLine(" = true;");
                builder.Append("                        __arg").Append(i.ToString(CultureInfo.InvariantCulture))
                    .Append(" = _context.").Append(GetTypeInfoPropertyName(parameter.ParameterType))
                    .AppendLine(".Read(reader)!;");
                ...
                if (parameter.LinkedMemberIndex >= 0 && parameter.LinkedMemberIndex < poco.Members.Length)
                {
                    ...
                    if (linkedMember.IsRequired)
                    {
                        if (useSeenMask)
                        {
                            builder.Append("                        seenMask |= 1UL << ")
                                .Append(parameter.LinkedMemberIndex.ToString(CultureInfo.InvariantCulture)).AppendLine(";");
                        }
                        else
                        {
                            builder.Append("                        if (seen is not null) seen[")
                                .Append(parameter.LinkedMemberIndex.ToString(CultureInfo.InvariantCulture))
                                .AppendLine("] = true;");
                        }
                    }

                    if (linkedMember.CanSet &&
                        SymbolEqualityComparer.Default.Equals(linkedMember.Type, parameter.ParameterType))
                    {
                        builder.Append("                        __memberValue")
                            .Append(parameter.LinkedMemberIndex.ToString(CultureInfo.InvariantCulture)).Append(" = __arg")
                            .Append(i.ToString(CultureInfo.InvariantCulture)).AppendLine(";");
                        builder.Append("                        __memberSeen")
                            .Append(parameter.LinkedMemberIndex.ToString(CultureInfo.InvariantCulture))
                            .AppendLine(" = true;");
                    }
                }
            ...
            for (var i = 0; i < poco.Members.Length; i++)
            {
                ...
                wroteAnyCondition = true;
                builder.Append("                    ").Append(prefix).Append("(string.Equals(name, \"")
                    .Append(EscapeStringLiteral(member.SerializedName)).Append("\", ").Append(comparison).AppendLine("))");
                builder.AppendLine("                    {");

                if (useSeenMask)
                {
                    builder.Append("                        const ulong bit = 1UL << ")
                        .Append(i.ToString(CultureInfo.InvariantCulture)).AppendLine(";");
                    builder.AppendLine(
                        "                        if (Options.DuplicateKeyHandling == TomlDuplicateKeyHandling.Error)");
                    builder.AppendLine("                        {");
                    builder.AppendLine(
                        "                            if ((seenMask & bit) != 0) throw reader.CreateException($\"Duplicate key '{name}' was encountered.\");");
                    builder.AppendLine("                        }");
                    ...
                    {
                        builder.AppendLine(
                            "                        if (Options.DuplicateKeyHandling == TomlDuplicateKeyHandling.Error) seenMask |= bit;");
                    }
                }
                else
                {
                    ...
                    builder.AppendLine(
                        "                            if (Options.DuplicateKeyHandling == TomlDuplicateKeyHandling.Error)");
                    builder.AppendLine("                            {");
                    builder.Append("                                if (seen[")
                        .Append(i.ToString(CultureInfo.InvariantCulture))
                        .AppendLine("]) throw reader.CreateException($\"Duplicate key '{name}' was encountered.\");");
                    builder.AppendLine("                            }");
                    builder.Append("                            seen[").Append(i.ToString(CultureInfo.InvariantCulture))
                        .AppendLine("] = true;");
                    builder.AppendLine("                        }");
                }
                ...
                builder.Append("                        __memberValue").Append(i.ToString(CultureInfo.InvariantCulture))
                    .Append(" = _context.").Append(GetTypeInfoPropertyName(member.Type)).AppendLine(".Read(reader)!;");
                builder.Append("                        __memberSeen").Append(i.ToString(CultureInfo.InvariantCulture))
                    .AppendLine(" = true;");
                builder.AppendLine("                        continue;");
                builder.AppendLine("                    }");
            }
            ...
            if (extensionData is not null)
            {
                ...
                builder.Append("                    __extensionData[name] = _context.")
                    .Append(GetTypeInfoPropertyName(extensionData.ValueType)).AppendLine(".Read(reader)!;");
                builder.AppendLine("                    continue;");
            }
            ...
            for (var i = 0; i < ctor.Parameters.Length; i++)
            {
                ...
                {
                    bucket = new List<(bool, int, string)>(1);
            ...
            {
                builder.Append("                            case 0x")
                    .Append(kvp.Key.ToString("X", CultureInfo.InvariantCulture)).AppendLine("UL:");
                builder.AppendLine("                            {");

                foreach (var action in kvp.Value)
                {
                    builder.Append("                                if (reader.PropertyNameEquals(\"")
                        .Append(EscapeStringLiteral(action.KeyName)).AppendLine("\"))");
                    builder.AppendLine("                                {");

                    if (action.IsParameter)
                    {
                        var parameter = ctor.Parameters[action.Index];
                        builder.Append("                                    if (__argSeen")
                            .Append(action.Index.ToString(CultureInfo.InvariantCulture)).AppendLine(
                                " && Options.DuplicateKeyHandling == TomlDuplicateKeyHandling.Error) throw reader.CreateException($\"Duplicate key '{reader.PropertyName}' was encountered.\");");
                        builder.Append("                                    __argSeen")
                            .Append(action.Index.ToString(CultureInfo.InvariantCulture)).AppendLine(" = true;");
                        builder.AppendLine("                                    reader.Read();");
                        builder.Append("                                    __arg")
                            .Append(action.Index.ToString(CultureInfo.InvariantCulture)).Append(" = _context.")
                            .Append(GetTypeInfoPropertyName(parameter.ParameterType)).AppendLine(".Read(reader)!;");
                        ...
                        if (parameter.LinkedMemberIndex >= 0 && parameter.LinkedMemberIndex < poco.Members.Length)
                        {
                                ...
                                SymbolEqualityComparer.Default.Equals(linkedMember.Type, parameter.ParameterType))
                            {
                                builder.Append("                                    __memberValue")
                                    .Append(parameter.LinkedMemberIndex.ToString(CultureInfo.InvariantCulture))
                                    .Append(" = __arg").Append(action.Index.ToString(CultureInfo.InvariantCulture))
                                    .AppendLine(";");
                                builder.Append("                                    __memberSeen")
                                    .Append(parameter.LinkedMemberIndex.ToString(CultureInfo.InvariantCulture))
                                    .AppendLine(" = true;");
                    ...
                    else
                    {
                        ...
                        if (useSeenMask)
                        {
                            ...
                            builder.AppendLine("                                    {");
                            builder.AppendLine(
                                "                                        if ((seenMask & bit) != 0) throw reader.CreateException($\"Duplicate key '{reader.PropertyName}' was encountered.\");");
                            builder.AppendLine("                                    }");
                            if (member.IsRequired)
                            {
                                builder.AppendLine("                                    seenMask |= bit;");
                        ...
                        else
                        {
                            ...
                            builder.Append("                                            if (seen[")
                                .Append(action.Index.ToString(CultureInfo.InvariantCulture)).AppendLine(
                                    "]) throw reader.CreateException($\"Duplicate key '{reader.PropertyName}' was encountered.\");");
                            builder.AppendLine("                                        }");
                            builder.Append("                                        seen[")
                                .Append(action.Index.ToString(CultureInfo.InvariantCulture)).AppendLine("] = true;");
                            builder.AppendLine("                                    }");
                        ...
                        else
                        {
                            builder.Append("                                    __memberValue")
                                .Append(action.Index.ToString(CultureInfo.InvariantCulture)).Append(" = _context.")
                                .Append(GetTypeInfoPropertyName(member.Type)).AppendLine(".Read(reader)!;");
                            builder.Append("                                    __memberSeen")
                                .Append(action.Index.ToString(CultureInfo.InvariantCulture)).AppendLine(" = true;");
                            builder.AppendLine("                                    continue;");
                        }
            ...
            for (var i = 0; i < ctor.Parameters.Length; i++)
            {
                ...
                {
                    builder.Append("                __arg").Append(i.ToString(CultureInfo.InvariantCulture)).Append(" = ")
                        .Append(parameter.DefaultValueExpression).AppendLine(";");
                    builder.AppendLine("            }");
                }
                else
                {
                    ...
                    builder.AppendLine("                var span = endTableSpan ?? tableStartSpan;");
                    builder.Append(
                            "                throw span is { } locatedSpan ? new TomlException(locatedSpan, $\"Missing required constructor parameter '")
                        .Append(keyName)
                        .Append("' when deserializing '{typeof(")
                        .Append(typeName)
            ...
            if (useObjectInitializerConstruction)
            {
                ...
                for (var i = 0; i < poco.Members.Length; i++)
                {
                    ...
                    needsTemplate = true;
                    finalInitializerAssignments.Add(member.MemberName + " = __memberValue" +
                    ...
                    {
                        templateValueExpression = "__memberSeen" + i.ToString(CultureInfo.InvariantCulture) +
                                                  " ? __memberValue" + i.ToString(CultureInfo.InvariantCulture) + " : " +
                                                  linkedParameterExpression;
                    }
                ...
                if (extensionData is { CanSet: true })
                {
                    ...
                    {
                        templateInitializerAssignments.Add(
                            extensionData.MemberName + " = " + extensionData.CreateExpression);
                    }
                }
                ...
                if (needsTemplate)
                {
                    ...
                    EmitPocoConstructionAssignment(builder, "__template", typeName, ctor.Parameters,
                        templateInitializerAssignments.ToImmutable(), wrapConstructionErrors);
                    ...
                    for (var i = 0; i < poco.Members.Length; i++)
                    {
                        ...
                        builder.Append("            if (!__memberSeen").Append(i.ToString(CultureInfo.InvariantCulture))
                            .AppendLine(")");
                        builder.AppendLine("            {");
                        builder.Append("                __memberValue").Append(i.ToString(CultureInfo.InvariantCulture))
                            .Append(" = __template.").Append(member.MemberName).AppendLine(";");
                        builder.AppendLine("            }");
                    }

                    if (extensionData is { CanSet: true })
                    {
                        builder.Append("            var __extensionDataValue = __template.")
                            .Append(extensionData.MemberName).AppendLine(";");
                        ...
                        builder.AppendLine("                {");
                        builder.Append("                    __extensionDataValue = ").Append(extensionData.CreateExpression)
                            .AppendLine(";");
                        builder.AppendLine("                }");
                        builder.AppendLine("                foreach (var __pair in __extensionData)");
                        builder.AppendLine("                {");
                ...
                EmitPocoConstructionAssignment(builder, "value", typeName, ctor.Parameters,
                    finalInitializerAssignments.ToImmutable(), wrapConstructionErrors);
                ...
                {
                    var nullInitializationMessage =
                        EscapeStringLiteral(
                            $"Extension data member '{extensionData.MemberName}' is null and cannot be initialized.");
                    builder.AppendLine("            if (__extensionData is not null && __extensionData.Count != 0)");
                    builder.AppendLine("            {");
                    builder.Append("                var __target = value.").Append(extensionData.MemberName)
                        .AppendLine(";");
                    builder.AppendLine("                if (__target is null)");
                    builder.AppendLine("                {");
                    builder.Append("                    throw new TomlException(\"").Append(nullInitializationMessage)
                        .AppendLine("\");");
                    builder.AppendLine("                }");
                    builder.AppendLine("                foreach (var __pair in __extensionData)");
                    builder.AppendLine("                {");
            ...
            {
                builder.Append("            ").Append(readNonNullableTypeName).AppendLine(" value;");
                EmitPocoConstructionAssignment(builder, "value", typeName, ctor.Parameters, ImmutableArray<string>.Empty,
                ...
                for (var i = 0; i < poco.Members.Length; i++)
                {
                    ...
                    builder.Append("            if (__memberSeen").Append(i.ToString(CultureInfo.InvariantCulture))
                        .AppendLine(")");
                    builder.AppendLine("            {");
                    builder.Append("                value.").Append(member.MemberName).Append(" = __memberValue")
                        .Append(i.ToString(CultureInfo.InvariantCulture)).AppendLine(";");
                    builder.AppendLine("            }");
                }
                ...
                if (extensionData is not null)
                {
                    ...
                    builder.AppendLine("                if (__target is null)");
                    builder.AppendLine("                {");
                    builder.Append("                    __target = ").Append(extensionData.CreateExpression)
                        .AppendLine(";");
                    builder.Append("                    value.").Append(extensionData.MemberName)
                        .AppendLine(" = __target;");
                    builder.AppendLine("                }");
                    builder.AppendLine("                foreach (var __pair in __extensionData)");
                    builder.AppendLine("                {");
            ...
            bool wrapConstructionErrors)
        {
            ...
            builder.AppendLine("            {");
            builder.AppendLine("                throw new TomlException($\"Failed to create an instance of '{typeof(" +
                               typeName + ").FullName}'.\", ex);");
            builder.AppendLine("            }");
        }
        ...
        private static void EmitMemberDispatch(StringBuilder builder, PocoShape poco, bool ignoreCase)
        {
            ...
            if (ignoreCase)
            {
                ...
                {
                    var member = poco.Members[i];
                    builder.Append("                    ").Append(i == 0 ? "if" : "else if")
                        .Append("(string.Equals(name, \"").Append(EscapeStringLiteral(member.SerializedName)).Append("\", ")
                        .Append(comparison).AppendLine("))");
                    builder.AppendLine("                    {");
                    if (useSeenMask)
                    {
                        if (member.IsRequired)
                    ...
                    else
                    {
                        ...
                        if (member.IsRequired)
                        {
                            ...
                            builder.AppendLine("                            }");
                            builder.Append("                            seen[")
                                .Append(i.ToString(CultureInfo.InvariantCulture)).AppendLine("] = true;");
                        }
                        else
                        {
                            builder.AppendLine(
                    ...
                    {
                        builder.Append("                        value.").Append(member.MemberName).Append(" = _context.")
                            .Append(GetTypeInfoPropertyName(member.Type)).AppendLine(".Read(reader)!;");
                    }
                ...
                if (poco.ExtensionData is { } extensionData)
                {
                    ...
                    builder.Append("                    var __extensionData = value.").Append(extensionData.MemberName)
                        .AppendLine(";");
                    builder.AppendLine("                    if (__extensionData is null)");
                    builder.AppendLine("                    {");
                    builder.Append("                        __extensionData = ").Append(extensionData.CreateExpression)
                        .AppendLine(";");
                    builder.Append("                        value.").Append(extensionData.MemberName)
                        .AppendLine(" = __extensionData;");
                    builder.AppendLine("                    }");
                    builder.Append("                    __extensionData[name] = _context.").Append(extensionValueTypeInfo)
                        .AppendLine(".Read(reader)!;");
                    builder.AppendLine("                    continue;");
                    return;
                }
            ...
            for (var i = 0; i < poco.Members.Length; i++)
            {
                ...
                {
                    bucket = new List<(int, PocoMember)>(1);
            ...
            foreach (var kvp in membersByHash.OrderBy(static pair => pair.Key))
            {
                ...
                {
                    builder.Append("                                if (reader.PropertyNameEquals(\"")
                        .Append(EscapeStringLiteral(member.SerializedName)).AppendLine("\"))");
                    builder.AppendLine("                                {");
                    if (useSeenMask)
                    {
                        if (member.IsRequired)
                    ...
                    else
                    {
                        ...
                        if (member.IsRequired)
                        {
                            ...
                            builder.AppendLine("                                        }");
                            builder.Append("                                        seen[")
                                .Append(index.ToString(CultureInfo.InvariantCulture)).AppendLine("] = true;");
                        }
                        else
                        {
                            builder.AppendLine(
                    ...
                    {
                        builder.Append("                                    value.").Append(member.MemberName)
                            .Append(" = _context.").Append(GetTypeInfoPropertyName(member.Type))
                            .AppendLine(".Read(reader)!;");
                    }
            ...
            if (poco.ExtensionData is { } extensionDataRead)
            {
                ...
                builder.AppendLine("                        var name = reader.PropertyName!;");
                builder.AppendLine("                        reader.Read();");
                builder.Append("                        var __extensionData = value.").Append(extensionDataRead.MemberName)
                    .AppendLine(";");
                builder.AppendLine("                        if (__extensionData is null)");
                builder.AppendLine("                        {");
                builder.Append("                            __extensionData = ").Append(extensionDataRead.CreateExpression)
                    .AppendLine(";");
                builder.Append("                            value.").Append(extensionDataRead.MemberName)
                    .AppendLine(" = __extensionData;");
                builder.AppendLine("                        }");
                builder.Append("                        __extensionData[name] = _context.").Append(extensionValueTypeInfo)
                    .AppendLine(".Read(reader)!;");
        ...
        private static void EmitWriteMember(StringBuilder builder, PocoMember member, int index, string memberTypeName,
            bool canBeNull, string? usedKeysVariable)
        {
            ...
            if (member.WriteIgnore == WriteIgnoreKind.WhenWritingNull)
            {
                if (canBeNull)
                {
                    ...
                    builder.Append(writeIndent).Append("writer.WritePropertyName(\"")
                        .Append(EscapeStringLiteral(member.SerializedName)).AppendLine("\");");
                    if (usedKeysVariable is not null)
                    {
                        builder.Append(writeIndent).Append(usedKeysVariable).Append("?.Add(\"")
                            .Append(EscapeStringLiteral(member.SerializedName)).AppendLine("\");");
                    }

                    builder.Append(writeIndent).Append("_context.").Append(GetTypeInfoPropertyName(member.Type))
                        .Append(".Write(writer, ").Append(localName).AppendLine(");");
                    builder.Append(openIndent).AppendLine("}");
                    return;
                }

                builder.Append(openIndent).Append("writer.WritePropertyName(\"")
                    .Append(EscapeStringLiteral(member.SerializedName)).AppendLine("\");");
                if (usedKeysVariable is not null)
                {
                    builder.Append(openIndent).Append(usedKeysVariable).Append("?.Add(\"")
                        .Append(EscapeStringLiteral(member.SerializedName)).AppendLine("\");");
                }

                builder.Append(openIndent).Append("_context.").Append(GetTypeInfoPropertyName(member.Type))
                    .Append(".Write(writer, ").Append(localName).AppendLine(");");
                return;
            }
            ...
            if (member.WriteIgnore == WriteIgnoreKind.WhenWritingDefault)
            {
                ...
                builder.Append(writeIndent).Append("_context.").Append(GetTypeInfoPropertyName(member.Type))
                    .Append(".Write(writer, ").Append(writeArgument).AppendLine(");");
                builder.Append(openIndent).AppendLine("}");
                return;
            }
            ...
            while (queue.Count != 0)
            {
                ...
                if (TryGetPocoShape(context, model, current, out var poco))
                {
                    ...
                    if (poco.Constructor is { IsValid: true } ctor)
                    {
                        ...
                        {
                            if (!IsSupportedMemberType(parameter.ParameterType))
                            {
                                context.ReportDiagnostic(Diagnostic.Create(
                                    UnsupportedMemberType,
                                    model.ContextSymbol.Locations.FirstOrDefault(),
                    ...
                    if (poco.ExtensionData is { } extensionData)
                    {
                        if (!IsSupportedMemberType(extensionData.ValueType))
                        {
                            context.ReportDiagnostic(Diagnostic.Create(
                                UnsupportedMemberType,
                                model.ContextSymbol.Locations.FirstOrDefault(),
        ...
        {
            public PocoMember(string memberName, string serializedName, ITypeSymbol type, int order,
                WriteIgnoreKind writeIgnore, bool isRequired, bool isCompilerRequired, bool canSet, bool isInitOnly)
            {
        ...
        {
            public PocoExtensionData(string memberName, ITypeSymbol memberType, ITypeSymbol valueType,
                string createExpression, bool canSet, bool isInitOnly, bool isCompilerRequired)
            {
                MemberName = memberName;
        ...
        {
            public PocoConstructor(IMethodSymbol? constructor, ImmutableArray<PocoConstructorParameter> parameters,
                string? errorMessage, bool setsRequiredMembers)
            {
        ...
        {
            public PocoShape(ImmutableArray<PocoMember> members, PocoExtensionData? extensionData,
                PocoConstructor? constructor, bool requiresGeneratedObjectInitializer)
            {
                Members = members;
            ...
            bool parameterlessConstructorSetsRequiredMembers)
        {
            ...
            var hasCompilerRequiredMembers = members.Any(static member => member.IsCompilerRequired) ||
                                             extensionData?.IsCompilerRequired == true;
        ...
        private sealed class PolymorphicShape
        {
            public PolymorphicShape(string? discriminatorPropertyName, ImmutableArray<PolymorphicDerivedType> derivedTypes,
                ITypeSymbol? defaultDerivedType, int? unknownDerivedTypeHandlingOverride)
            {
        ...
        private static bool TryGetPocoShape(SourceProductionContext context, ContextModel model, ITypeSymbol type,
            out PocoShape shape)
        {
            shape = null!;
            ...
            if (named.TypeKind == TypeKind.Class)
            {
                ...
                else
                {
                    ...
                    else if (publicConstructors.FirstOrDefault(static ctor => ctor.Parameters.Length == 0) is
                             { } parameterlessConstructor)
                    {
                        selectedConstructor = null;
                        parameterlessConstructorSetsRequiredMembers = HasAttribute(parameterlessConstructor,
            ...
            foreach (var member in named.GetMembers().OfType<IPropertySymbol>())
            {
                ...
                if (IsExtensionData(member))
                {
                    ...
                    extensionData = new PocoExtensionData(member.Name, member.Type, extensionValueType, createExpression,
                        canSet, isInitOnly, isCompilerRequired);
                    continue;
                }
                ...
                members.Add(new PocoMember(member.Name, serializedName, member.Type, order, ignore.WriteIgnore, required,
                    isCompilerRequired, canSet, isInitOnly));
            }
            ...
            foreach (var member in named.GetMembers().OfType<IFieldSymbol>())
            {
                ...
                var required = IsRequired(member);
                members.Add(new PocoMember(member.Name, serializedName, member.Type, order, ignore.WriteIgnore, required,
                    member.IsRequired, true, false));
            }

            PocoConstructor? constructorModel = null;
            if (constructorError is not null && selectedConstructor is null)
            {
                constructorModel = new PocoConstructor(null, ImmutableArray<PocoConstructorParameter>.Empty,
            ...
            else if (selectedConstructor is not null)
            {
                ...
                if (selectedConstructor.Parameters.Length > 0)
                {
                    ...
                    var parameterComparer = model.Options.PropertyNameCaseInsensitive == true
                        ? StringComparer.OrdinalIgnoreCase
                        : StringComparer.Ordinal;
                    var parameterKeys = new HashSet<string>(parameterComparer);
                    var parameters =
                        ImmutableArray.CreateBuilder<PocoConstructorParameter>(selectedConstructor.Parameters.Length);
                    ...
                    for (var i = 0; i < selectedConstructor.Parameters.Length; i++)
                    {
                        ...
                        var linkedMemberIndex = memberIndexByClrName.TryGetValue(parameterName, out var linkedIndex)
                            ? linkedIndex
                            : -1;
                        ...
                        {
                            constructorError ??=
                                $"Constructor parameter name collision for key '{keyName}' on type '{type.ToDisplayString()}'.";
                        }

                        var hasDefaultValue = parameter.HasExplicitDefaultValue;
                        string? defaultValueExpression = null;
                        if (hasDefaultValue && !TryGetDefaultValueExpression(parameter.Type, parameter.ExplicitDefaultValue,
                                out defaultValueExpression))
                        {
                            constructorError ??=
                                $"Constructor parameter '{parameterName}' on type '{type.ToDisplayString()}' has an unsupported default value.";
                            hasDefaultValue = false;
                        }

                        parameters.Add(new PocoConstructorParameter(keyName, parameterName, parameter.Type, hasDefaultValue,
                            defaultValueExpression, linkedMemberIndex));
                    }

                    constructorModel = new PocoConstructor(selectedConstructor, parameters.ToImmutable(), constructorError,
                        setsRequiredMembers);
                    shape = new PocoShape(
                        membersSoFar,
                        extensionData,
                        constructorModel,
                        RequiresGeneratedObjectInitializer(membersSoFar, extensionData, constructorModel,
                            parameterlessConstructorSetsRequiredMembers));
                    return true;
                }

                constructorModel = new PocoConstructor(selectedConstructor, ImmutableArray<PocoConstructorParameter>.Empty,
                    constructorError, setsRequiredMembers);
            }
            ...
            shape = new PocoShape(
                ...
                extensionData,
                constructorModel,
                RequiresGeneratedObjectInitializer(finalMembers, extensionData, constructorModel,
                    parameterlessConstructorSetsRequiredMembers));
            return true;
        }

        private static bool TryGetPolymorphicShape(SourceProductionContext context, ContextModel model, ITypeSymbol type,
            out PolymorphicShape shape, bool reportDiagnostics = true)
        {
            shape = null!;
            ...
            foreach (var attr in named.GetAttributes())
            {
                ...
                if (attrName == "global::Tomlyn.Serialization.TomlDerivedTypeAttribute")
                {
                        ...
                        if (!IsAssignableTo(defaultType, type))
                        {
                            if (reportDiagnostics)
                            {
                                context.ReportDiagnostic(Diagnostic.Create(
                                    ...
                                    model.ContextSymbol.Locations.FirstOrDefault(),
                                    type.ToDisplayString(),
                                    $"Derived type '{defaultType.ToDisplayString(SymbolDisplayFormat.FullyQualifiedFormat)}' is not assignable to base type '{type.ToDisplayString(SymbolDisplayFormat.FullyQualifiedFormat)}'."));
                            }

                            continue;
                        }

                        if (defaultType is INamedTypeSymbol defaultNamed &&
                            (defaultNamed.TypeKind != TypeKind.Class || defaultNamed.IsAbstract))
                        {
                            if (reportDiagnostics)
                            {
                                context.ReportDiagnostic(Diagnostic.Create(
                                    ...
                                    model.ContextSymbol.Locations.FirstOrDefault(),
                                    type.ToDisplayString(),
                                    $"Derived type '{defaultType.ToDisplayString(SymbolDisplayFormat.FullyQualifiedFormat)}' must be a non-abstract class."));
                            }
                             ...
                             attr.ConstructorArguments[0].Value is ITypeSymbol derivedType)
                    {
                        ...
                        AddDerivedType(context, model, type, derived, discriminatorSet, derivedTypeSet, derivedType,
                            discriminator);
                    }
                    else
                    {
                ...
                string discriminator)
            {
                ...
                if (derivedType is INamedTypeSymbol derivedNamed &&
                    (derivedNamed.TypeKind != TypeKind.Class || derivedNamed.IsAbstract))
                {
                    if (reportDiagnostics)
                    {
                        context.ReportDiagnostic(Diagnostic.Create(
        ...
        private static bool TryGetExtensionDataValueType(ITypeSymbol type, out ITypeSymbol valueType)
        {
            ...
            static bool IsStringDictionaryInterface(INamedTypeSymbol iface, out ITypeSymbol dictionaryValueType)
            {
                ...
                if (iface.ConstructedFrom.ToDisplayString(SymbolDisplayFormat.FullyQualifiedFormat) !=
        ...
        private static bool TryGetExtensionDataCreateExpression(ITypeSymbol memberType, ITypeSymbol valueType,
            out string createExpression)
        {
            createExpression = null!;

            var memberTypeNoOuterNullability = memberType.WithNullableAnnotation(NullableAnnotation.NotAnnotated);
            var memberTypeNameNoNullability =
                memberTypeNoOuterNullability.ToDisplayString(SymbolDisplayFormat.FullyQualifiedFormat);
            ...
            {
                if (named.TypeKind == TypeKind.Struct ||
                    named.Constructors.Any(static c =>
        ...
        {
            if (namingPolicyExpression is not null &&
                TryConvertKnownName(parameterName, namingPolicyExpression, out var converted))
            {
                return converted;
            }
        ...
        private static bool TryGetDictionaryKeyValueTypes(ITypeSymbol type, out ITypeSymbol keyType,
        ...
        private static string GetSerializedName(ISymbol member, string memberName, string? namingPolicyExpression)
        {
            ...
            if (namingPolicyExpression is not null &&
                TryConvertKnownName(memberName, namingPolicyExpression, out var converted))
            {
                return converted;
            }
        ...
        {
            var policy = namingPolicyExpression switch
            {
        ...
        private static IgnoreBehavior GetIgnoreBehavior(ISymbol symbol)
        {
            ...
            if (toml is not null)
            {
                ...
                {
                    1 => new IgnoreBehavior(false, WriteIgnoreKind.WhenWritingNull),
                    2 => new IgnoreBehavior(false, WriteIgnoreKind.WhenWritingDefault),
                    _ => new IgnoreBehavior(true, WriteIgnoreKind.None)
                };
            }

            return new IgnoreBehavior(false, WriteIgnoreKind.None);
        }
        ...
        private static bool DerivesFromTomlSerializerContext(INamedTypeSymbol symbol)
        {
            for (var current = symbol; current is not null; current = current.BaseType)
            {
                if (current.ToDisplayString(SymbolDisplayFormat.FullyQualifiedFormat) ==
                    "global::" + TomlSerializerContextMetadataName)
                {
                    return true;
                }
            }
        ...
        private static bool IsTomlSerializableAttribute(AttributeData attribute)
        {
            return attribute.AttributeClass?.ToDisplayString(SymbolDisplayFormat.FullyQualifiedFormat) ==
                   "global::" + TomlSerializableAttributeMetadataName;
        }
        ...
        {
            return attribute.AttributeClass?.ToDisplayString(SymbolDisplayFormat.FullyQualifiedFormat) ==
                   "global::" + TomlSourceGenerationOptionsAttributeMetadataName;
        }
        ...
        private static void ApplyTomlSourceGenerationOptionsAttribute(AttributeData attribute, SourceGenOptions options)
        {
            foreach (var namedArgument in attribute.NamedArguments)
            {
                ...
                {
                    case "WriteIndented":
                        if (value.Value is bool wi)
                        {
                            options.WriteIndented = wi;
                        }

                        break;
                    case "IndentSize":
                        if (value.Value is int indent)
                        {
                    ...
                    case "PropertyNameCaseInsensitive":
                        if (value.Value is bool pnci)
                        {
                            options.PropertyNameCaseInsensitive = pnci;
                        }
                    ...
                    case "DefaultIgnoreCondition":
                        if (value.Value is int dic)
                        {
                            options.DefaultIgnoreCondition = dic;
                        }

                        break;
                    case "DuplicateKeyHandling":
                        if (value.Value is int dkh)
                        {
                            options.DuplicateKeyHandling = dkh;
                        }

                        break;
                    case "MaxDepth":
                        if (value.Value is int maxDepth)
                        {
                        ...
                        break;
                    case "MappingOrder":
                        if (value.Value is int mappingOrder)
                        {
                            options.MappingOrder = mappingOrder;
                        }

                        break;
                    case "DottedKeyHandling":
                        if (value.Value is int dottedKeyHandling)
                        {
                            options.DottedKeyHandling = dottedKeyHandling;
                        }

                        break;
                    case "RootValueHandling":
                        if (value.Value is int rootValueHandling)
                        {
                            options.RootValueHandling = rootValueHandling;
                        }

                        break;
                    case "RootValueKeyName":
                        if (value.Value is string rootValueKeyName)
                        {
                            options.RootValueKeyName = rootValueKeyName;
                        }

                        break;
                    case "InlineTablePolicy":
                        if (value.Value is int inlineTablePolicy)
                        {
                            options.InlineTablePolicy = inlineTablePolicy;
                        }

                        break;
                    case "TableArrayStyle":
                        if (value.Value is int tableArrayStyle)
                        {
                            options.TableArrayStyle = tableArrayStyle;
                        }
        ...
        private static void ValidateConverters(SourceProductionContext context, ContextModel model)
        {
            ...
            foreach (var type in model.Options.ConverterTypes)
            {
                ...
                if (named.TypeKind != TypeKind.Class)
                {
                    context.ReportDiagnostic(Diagnostic.Create(InvalidConverterType,
                        model.ContextSymbol.Locations.FirstOrDefault(), type.ToDisplayString(),
                        "Converters must be classes."));
                    continue;
                }
                ...
                if (named.IsAbstract)
                {
                    context.ReportDiagnostic(Diagnostic.Create(InvalidConverterType,
                        model.ContextSymbol.Locations.FirstOrDefault(), type.ToDisplayString(),
                        "Converters must not be abstract."));
                    continue;
                }
                ...
                if (!DerivesFrom(named, "global::" + TomlConverterMetadataName))
                {
                    context.ReportDiagnostic(Diagnostic.Create(InvalidConverterType,
                        model.ContextSymbol.Locations.FirstOrDefault(), type.ToDisplayString(),
                        $"Converters must derive from {TomlConverterMetadataName}."));
                    continue;
                }

                if (!named.Constructors.Any(static c =>
                        c.Parameters.Length == 0 && c.DeclaredAccessibility == Accessibility.Public))
                {
                    context.ReportDiagnostic(Diagnostic.Create(InvalidConverterType,
                        model.ContextSymbol.Locations.FirstOrDefault(), type.ToDisplayString(),
                        "Converters must have a public parameterless constructor."));
                }
            }
        }
        ...
        private static void ValidateSourceGenerationOptions(SourceProductionContext context, ContextModel model)
        {
            ...
            ValidateEnumOption(context, model, "NewLine", model.Options.NewLine, TryGetTomlNewLineKindExpression,
                v => model.Options.NewLine = v);
            ValidateEnumOption(context, model, "DefaultIgnoreCondition", model.Options.DefaultIgnoreCondition,
                TryGetTomlIgnoreConditionExpression, v => model.Options.DefaultIgnoreCondition = v);
            ValidateEnumOption(context, model, "DuplicateKeyHandling", model.Options.DuplicateKeyHandling,
                TryGetTomlDuplicateKeyHandlingExpression, v => model.Options.DuplicateKeyHandling = v);
            ...
            ValidateEnumOption(context, model, "MappingOrder", model.Options.MappingOrder,
                TryGetTomlMappingOrderPolicyExpression, v => model.Options.MappingOrder = v);
            ValidateEnumOption(context, model, "DottedKeyHandling", model.Options.DottedKeyHandling,
                TryGetTomlDottedKeyHandlingExpression, v => model.Options.DottedKeyHandling = v);
            ValidateEnumOption(context, model, "RootValueHandling", model.Options.RootValueHandling,
                TryGetTomlRootValueHandlingExpression, v => model.Options.RootValueHandling = v);
            ValidateEnumOption(context, model, "InlineTablePolicy", model.Options.InlineTablePolicy,
                TryGetTomlInlineTablePolicyExpression, v => model.Options.InlineTablePolicy = v);
            ValidateEnumOption(context, model, "TableArrayStyle", model.Options.TableArrayStyle,
                TryGetTomlTableArrayStyleExpression, v => model.Options.TableArrayStyle = v);

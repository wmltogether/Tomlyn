...
namespace Tomlyn.Tests
{
    public sealed class NewApiConstructorBindingValidationTests
    {
        ...
        {
            var options = TomlSerializerOptions.Default with { PropertyNameCaseInsensitive = true };

            Assert.Throws<TomlException>(() =>
                TomlSerializer.Serialize(new CollisionType(1, 2), options)
            );
        }
    }
}
